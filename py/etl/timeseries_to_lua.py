...
def extend_datafiles(encounter_id, output_path_lua, output_path_py, player_class, spec, timeseries, names, generate_lua):
    ...
    if generate_lua:
        ...
        new_lua_body += generate_lua_db_body(player_class, master_frames)
...
def init_create_datafiles(encounter_id, output_path_lua, output_path_py, player_class, spec, timeseries, names):
    ...
    # data
    # lines = generate_lua_db_body(player_class, master_frames)
...
def generate_lua_body_for_encounter(player_class, spec, encounter_id, frame, names):
    ...
    for i in range(len(frame)):
        ...
        if frame[i] is not None:
            ...
            print(f"Error: missing data for {player_class}-{spec} encounter: {encounter_id}, rank: {i + 1}")
            data_str = "0"
...
def generate_lua_db_body(player_class, master_frames):
    final_strings = []
    for spec, data in master_frames.items():
        for encounter_id, entry in data.items():
            frame = entry["data"]
            names = entry["names"]
